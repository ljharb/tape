...
Test.prototype.rejects = function (promiseFn, expected, msg, extra) {
    ...
    var resolved = Promise.resolve(promiseFn);
    if (resolved !== promiseFn || (typeof promiseFn.then !== 'function' && typeof promiseFn['catch'] !== 'function')) {
        throw new TypeError('`rejects` requires a native Promise, or a function that returns one');
    }
    var self = this;
    return resolved.then(
        function () {
            self.fail('expected rejection, got fulfillment');
        },
        function (reason) {
            self['throws'](
